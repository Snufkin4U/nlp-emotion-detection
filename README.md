...
* `emotion\_nlp\_assignment.ipynb` - complete executed submission notebook
* `data/train.txt`, `data/val.txt`, `data/test.txt` - original Kaggle dataset splits
* `requirements.txt` - required Python packages
* `DATASET\_SOURCE.md` - dataset source, attribution, and license information

## Running the Notebook

Open `emotion\_nlp\_assignment.ipynb` in Google Colab or Jupyter and select **Run all**. If the `data` directory is unavailable, the notebook attempts to download the public dataset archive directly from Kaggle.
